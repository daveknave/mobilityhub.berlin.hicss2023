...
class LargeImage(blocks.StructBlock):
        ImageChooserBlock ( 
...
class MediumImageText(blocks.StructBlock):

    image = ImageChooserBlock()
    text = blocks.CharBlock(max_length=300, required=False)

    class Meta:
        template = "streams/medium_image_text.html"
        icon = "image"
        label = "Medium Image & Text"


class CallToActionBlock(blocks.StructBlock):

    title = blocks.CharBlock(max_length=200, required=False, help_text='Max length of 200 characters.')
    link = Link()
...
#beispiel für titel und text:
"""
class RichtTextWithTitleBlock(blocks.StructBlock):

    title = blocks.CharBlock(max_length=50)
    content = blocks.RichtTextBlock(features=[])

    class Meta:
        template = "streams/simple:richttext_block.html"

in template aber self.title und self.content anpassen
